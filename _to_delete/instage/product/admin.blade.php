...
<div class="ovl" id="tgt-ovl"><div class="modal" style="width:560px">
  ...
  <div class="mbody">
    ...
    <div class="fbool"><input type="checkbox" id="tgt-active" checked> Active (receives the nightly push)</div>
...
async function loadTargets() {
  try {
    ...
    $('#tgt-rows').innerHTML = (d.data || []).length ? d.data.map((t) =>
      ...
      + '<td class="mut" style="max-width:220px;overflow:hidden;text-overflow:ellipsis">' + esc(t.url) + '</td>'
...
$('#tgt-save').onclick = async () => {
  const body = { name: $('#tgt-name').value.trim(), url: $('#tgt-url').value.trim(),
    secret: $('#tgt-secret').value.trim() || null, active: $('#tgt-active').checked, events: ['attendance.daily'] };
