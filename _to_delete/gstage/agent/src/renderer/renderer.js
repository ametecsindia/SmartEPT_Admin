...
function renderFlags(p) {
  if (!p) return;
  const items = [
    ['Tracking', p.tracking_enabled],
    [p.screenshot_enabled && p.screenshot_interval_enabled === false ? 'Screenshots (violations only)' : 'Screenshots', p.screenshot_enabled],
...
const BREAK_LABELS = { TEA: 'Tea', LUNCH: 'Lunch', CUSTOM: 'Break' };
function renderBreak(active) {
  document.querySelectorAll('.breaks button[data-b]').forEach((b) => {
    b.classList.toggle('onbreak', !!active && active.type === b.dataset.b);
  });
  $('btn-break-end').disabled = !active;
  if (active) {
    const t = active.started_at ? active.started_at.slice(11, 16) : '';
    $('break-status').innerHTML = '<b>● On ' + (BREAK_LABELS[active.type] || active.type)
      + ' break' + (t ? ' since ' + t : '') + '</b> — press “End current break” when you are back.';
  } else {
    $('break-status').textContent = '';
  }
}
function flashBreakStatus(msg) {
  const el = $('break-status');
  el.textContent = msg;
  clearTimeout(el._t);
  el._t = setTimeout(() => renderBreak(window._activeBreak || null), 3500);
}
document.querySelectorAll('.breaks button[data-b]').forEach((b) => {
  b.addEventListener('click', async () => {
    const res = await window.smartept.break({ action: 'START', break_type: b.dataset.b });
    if (res && res.ok) return; // state push repaints the buttons
    if (res && res.reason === 'already_on_break') flashBreakStatus('This break is already running — press “End current break” to finish it.');
  });
});
$('btn-break-end').addEventListener('click', async () => {
  const res = await window.smartept.break({ action: 'END' });
  if (res && res.reason === 'no_break') flashBreakStatus('No break is running right now.');
});
...
window.smartept.onState((s) => {
  ...
  window._activeBreak = s.active_break || null;
  renderBreak(window._activeBreak);
});
...
window.smartept.onGate((g) => {
  const wall = document.getElementById('gate-wall');
  if (!wall) return;
  if (g && g.gate_required && !g.open) {
    document.getElementById('gate-msg').textContent = g.message
      || 'Your SmartEPT session begins the moment your door punch reaches us.';
    const nm = document.getElementById('emp-name');
    document.getElementById('gate-name').textContent = nm ? nm.textContent : '—';
