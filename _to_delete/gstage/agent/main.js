...
const { stopActiveWindowHelper } = require('./src/tracking/activeWindow');
...
let tracking = false;
let gatePolling = null;
let lastGate = null;
let activityBuffer = [];
let timers = [];
let lastSync = { online: false, pending: 0 };
let presenceCurrent = null; // { status, startedAt, meta }
let activeBreak = null;     // { type, started_at } — the ONE break running right now
let presenceFlushTimer = null;

function brandIcon(file) {
  // Real SmartEPT brand icon shipped in assets/ (17-Jul); falls back to the
  // embedded pixel mark if the file is ever missing so the agent never breaks.
  try {
...
function createWindow() {
  win = new BrowserWindow({
    ...
    icon: brandIcon('icon.png') || undefined,
...
// 17-Jul: real brand icon from assets/tray.png; base64 mark kept as fallback.
...
function createTray() {
  if (tray) return;
  tray = new Tray(brandIcon('tray.png') || TRAY_ICON_FALLBACK);
...
ipcMain.handle('break', (_e, { action, break_type }) => {
  if (!tracking) return { ok: false, reason: 'not_tracking' };
  const type = break_type || 'CUSTOM';

  if (action === 'START') {
    // One break at a time; the same button twice is a no-op, not a duplicate.
    if (activeBreak && activeBreak.type === type) {
      return { ok: false, reason: 'already_on_break', active_break: activeBreak };
    }
    activeBreak = { type, started_at: fmtDate(new Date()) };
    store.enqueue('POST', '/agent/break-event', {
      device_uuid: store.deviceUuid(), action: 'START', break_type: type,
      source: 'MANUAL', occurred_at: activeBreak.started_at,
    });
  } else {
    if (!activeBreak) return { ok: false, reason: 'no_break' };
    // END carries the RUNNING break's type (the server closes all open ones anyway).
    store.enqueue('POST', '/agent/break-event', {
      device_uuid: store.deviceUuid(), action: 'END', break_type: activeBreak.type,
  ...
  sync.drain();
  pushState();
  return { ok: true, active_break: activeBreak };
});
...
// GATE-TO-PC (USP): the agent will not begin a work session until SmartEPT
// confirms a door/biometric IN punch for the day. startTracking() now checks
// the gate first; if closed it shows the Gate Wall and polls until it opens.
function startTracking() {
  if (tracking) return;
  ensureGateThenBegin();
}

async function ensureGateThenBegin() {
  try {
    const g = await api.request('GET', '/agent/gate-status');
    lastGate = g;
    if (g && g.gate_required && !g.open) {
      pushGate(g);
      if (!gatePolling) {
        gatePolling = setInterval(() => ensureGateThenBegin().catch(() => {}), 15000);
      }
      return; // wall stays up; do not start tracking
    }
  } catch (e) {
    // If the gate cannot be checked (server/network), fail SAFE for the USP:
    // when the last known state required a closed gate, keep waiting; otherwise
    // (never gated) proceed so a gate outage does not lock out normal clients.
    if (lastGate && lastGate.gate_required && !lastGate.open) { return; }
  }
  if (gatePolling) { clearInterval(gatePolling); gatePolling = null; }
  pushGate(lastGate && lastGate.gate_required ? { ...lastGate, open: true } : { gate_required: false, open: true });
  _beginTracking();
}

function pushGate(g) {
  if (win && !win.isDestroyed()) win.webContents.send('gate', g || { gate_required: false, open: true });
}

function _beginTracking() {
  if (tracking) return;
...
function stopTracking() {
  ...
  activeBreak = null;
  if (gatePolling) { clearInterval(gatePolling); gatePolling = null; }
  stopActiveWindowHelper(); // kill the PowerShell helper — no orphan processes
...
function summarizePolicy(bundle) {
  ...
  return {
    ...
    // 17-Jul capture-mode: interval_enabled false = violations-only screenshots.
    // Absent column (older server) counts as true so behaviour never regresses.
    screenshot_interval_enabled: !p.screenshot || p.screenshot.interval_enabled === undefined
      || !!p.screenshot.interval_enabled,
