...
class ScreenshotEngine {
  ...
  start(policy) {
    ...
    // Retry loop always runs — pending violation captures must still drain.
    this._retryTimer = setInterval(() => this.flushPending().catch(() => {}), 60_000);

    // 17-Jul capture-mode: with timed captures OFF the policy is
    // "violations only" — no interval/first-boot captures at all.
    if (policy.screenshot_interval_enabled === false) return;
