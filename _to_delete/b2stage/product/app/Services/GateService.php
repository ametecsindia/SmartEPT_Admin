    ...
    public function statusFor(Employee $employee): array
    {
        ...
        if (! $punch) {
            $gateAttendance = EmployeeAttendanceLog::withoutGlobalScopes()
                ...
                // MANUAL = an admin/HR punch recorded on the attendance sheet with a
                // reason (audited) — a legitimate supervisor override that opens the gate.
                ->whereIn('source', ['BIOMETRIC', 'API', 'MANUAL'])
